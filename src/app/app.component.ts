...
export class AppComponent implements AfterViewInit, OnDestroy {
  ...
  readonly scenarioMessage = signal('Select a scenario or draw a polygon to start.');
  ...
  commodity = 'All';
  voyageStatus = 'All';
  movementFilters: Record<MovementType, boolean> = { Entered: true, Exited: true, Remained: true };

  readonly commodities = computed(() => ['All', ...new Set(this.allVessels().map(v => v.commodity))]);
  readonly statuses = computed(() => ['All', ...new Set(this.allVessels().map(v => v.voyageStatus))]);
  ...
  readonly scenarios: SavedScenario[] = [
    {
      ...
      commodity: 'All',
      voyageStatus: 'All',
      polygonJson: JSON.stringify({"rings":[[[-97,26],[-97.2,27],[-97,28.5],[-96,29.8],[-95,30],[-93,29.5],[-91,29],[-90,27.5],[-91,26],[-93,25.5],[-95.5,25.8],[-97,26]]]})
    },
    {
      id: 'crude-corridor',
      name: 'Crude Corridor',
      ...
      commodity: 'Crude',
      voyageStatus: 'All',
      polygonJson: JSON.stringify({"rings":[[[-95.5,27.5],[-95.8,28],[-95.5,29],[-94.8,29.2],[-94,28.8],[-93.8,28],[-94.2,27.5],[-95,27],[-95.5,27.5]]]})
    },
    {
      id: 'discharge-zone',
      name: 'Discharge Zone',
      ...
      voyageStatus: 'Watch',
      polygonJson: JSON.stringify({"rings":[[[-92,28],[-92.3,28.3],[-92,28.8],[-91.5,28.9],[-91,28.5],[-91.2,28],[-91.5,27.8],[-92,28]]]})
    },
    {
      id: 'entry-point',
      name: 'Entry Point',
      ...
      polygonJson: JSON.stringify({"rings":[[[-96.5,26.5],[-96.8,26.8],[-96.5,27.5],[-95.8,27.4],[-95.5,26.8],[-95.8,26.2],[-96.5,26.5]]]})
    }
  ];
  ...
  drawPolygon(): void {
    this.scenarioMessage.set('Click on the map to draw polygon corners. Double-click to finish.');
  ...
  runAnalysis(): void {
    const chosenMovements = (Object.keys(this.movementFilters) as MovementType[])
      .filter(value => this.movementFilters[value]);

    const results = this.allVessels().filter(vessel => {
      const matchesAttributes = chosenMovements.includes(vessel.movement)
        && (this.commodity === 'All' || vessel.commodity === this.commodity)
        && (this.voyageStatus === 'All' || vessel.voyageStatus === this.voyageStatus);

      if (!matchesAttributes || !this.currentPolygon) return matchesAttributes;
      const point = new Point({ longitude: vessel.longitude, latitude: vessel.latitude, spatialReference: { wkid: 4326 } });
  ...
  loadScenario(scenarioId: string): void {
    ...
    scenario.movementFilters.forEach(value => this.movementFilters[value] = true);

    if (scenario.polygonJson) {
      try {
        this.currentPolygon = Polygon.fromJSON(JSON.parse(scenario.polygonJson));
        this.polygonLayer.removeAll();
        this.polygonLayer.add(new Graphic({ geometry: this.currentPolygon, symbol: this.polygonSymbol() }));
  ...
  addVoyage(vessel: Vessel): void {
    ...
    this.voyageMessage.set(`Voyage: ${vessel.name} (${vessel.imo}) - Load Date: ${loadDate}`);
  }
  ...
  private configureHover(): void {
    ...
    this.hoverHandle = this.view.on('pointer-move', event => {
      ...
      pending = true;
      this.view.hitTest(event, { include: this.vesselLayer }).then(response => {
        const graphic = response.results.find(result => result.type === 'graphic')?.graphic;
        this.mapViewNode.nativeElement.style.cursor = graphic ? 'pointer' : 'default';
        if (graphic) {
          this.view?.openPopup({
            features: [graphic],
  ...
  private renderMap(): void {
    ...
    for (const vessel of this.displayedVessels()) {
      ...
      if (vessel.track && vessel.track.length > 1) {
        const trackCoords = vessel.track.map(tp => [tp.longitude, tp.latitude]);
        const polyline = new Polyline({ paths: [trackCoords], spatialReference: { wkid: 4326 } });
        ...
        this.trackLayer.add(new Graphic({ 
          geometry: polyline, 
          symbol: lineSymbol,
        ...
        for (let i = 0; i < vessel.track.length; i++) {
          ...
          const point = new Point({ longitude: tp.longitude, latitude: tp.latitude });
          const color = this.commodityColor(vessel.commodity);
      ...
      // Draw vessel marker
      const color = this.commodityColor(vessel.commodity);
      const point = new Point({ longitude: vessel.longitude, latitude: vessel.latitude });
      const marker = new SimpleMarkerSymbol({
        style: 'triangle', color, size: 15, angle: vessel.heading,
        outline: { color: [255, 255, 255, 0.95], width: 1.5 }
      });
      this.vesselLayer.add(new Graphic({
        geometry: point,
        symbol: marker,
        attributes: { ...vessel },
        popupTemplate: {
